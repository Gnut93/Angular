...
export class AppComponent {
  ...
  listNhaKH: NhaKhoaHoc[] = [
    { id: 1, ho: 'Tạ Quang', ten: 'Bửu', tuoi: 76, sinh: 1910, mat: 1986 },
    { id: 2, ho: 'Trần Đại', ten: 'Nghĩa', tuoi: 84, sinh: 1913, mat: 1997 },
    { id: 3, ho: 'Lê Văn', ten: 'Thiêm', tuoi: 72, sinh: 1918, mat: 1991 },
    { id: 4, ho: 'Nguyễn Văn', ten: 'Hiệu', tuoi: 84, sinh: 1938, mat: 2022 },
    { id: 5, ho: 'Hoàng', ten: 'Tuỵ', tuoi: 92, sinh: 1927, mat: 2019 },
  ];
  ...
  render() {
    let codehtml = this.listNhaKH
      .map((nkh) => {
        return `
      <p> ${nkh.id}. ${nkh.ho} ${nkh.ten} -  Tuổi: ${nkh.tuoi} ( ${nkh.sinh} - ${nkh.mat} )</p>
  ...
  renderSP() {
    ...
    this.listSP.forEach((sp) => {
      codehtml += `
      <div class='card text-center mb-4 sp'>
      <div class='card-header h5' style = "color:blue;">${sp.tensp}</div>
      <div class='card-body'>
      <img class='hinhsp mb-4' src="${
        sp.urlImage
      }" style="width:200px;height:350px">
      <p class='card-text'>${sp.mota}</p>
      <a href='#' class='btn btn-primary'>Xem Chi Tiết</a>
      </div>
      <div class='card-footer' style="color:tomato;
      font-weight:600;font-size:18px;">${sp.giasp.toLocaleString(
        'vi'
      )} VNĐ</div>
      </div>
