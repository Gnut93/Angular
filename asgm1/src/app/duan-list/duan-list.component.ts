...
export class DuanListComponent implements OnInit {
  ...
  listFilter: string = '';
  listDuAnFiltered: DuAn[] = []; //
  ngOnInit(): void {
    this.listDuAnFiltered = this.listDuAn;
  }
  filterDuAnByName() {
    this.listDuAn = this.listDuAnFiltered.filter((duan) => {
      return duan.tenDuAn.toLowerCase().includes(this.listFilter.toLowerCase());
    });
  }
}
